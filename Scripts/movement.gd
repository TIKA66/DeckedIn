...
#REVIEW - do we need this? cos the player is only handed valid movememnt options
...
func move_player() -> void:
	$Card_System.available_movement -= movement_cost # subtract movement points used to move
	player_position = selected_room #set player position to new room
...
func place_artefact() -> void:
	$Artefact.random_artefact_room = randi_range(0, adjacent_rooms.size()) #set artefact room to a random adjacent room
	print("artefact position chosen ", $Artefact.random_artefact_room, "nd spot out of adjacent rooms: ", adjacent_rooms) #DEBUGGING check which room it set to
...
#	reroll if artefact sets to the same spot as player
	if $Artefact.artefact_position == player_position: # if the artefact is in the players room, reroll
...
func current_room() -> void:
	...
	match Map.map[player_position]["type"]: #determine which type of room to start displaying
		...
		"monster":
			$Rooms.monster() #spawn a monster
		"market":
			$Rooms.market() #load in market items and process sale
		"ending":
			$Rooms.ending() #check for artefact posession
